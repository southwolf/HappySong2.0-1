  ...
  class DynamicsApi < Grape::API
    ...
    resources :dynamics do
      ...
      get '/time_dynamics' do
        ...
        if user_id.nil?
          authenticate!
          dynamics = current_user.dynamics.order(:created_at => :desc).select {|dynamic| DateTime.parse(dynamic.created_at.to_s).strftime('%Y-%-m') == time }
          present paginate(Kaminari.paginate_array(dynamics)), with: ::Entities::Dynamic
        else
          user = User.find(user_id)
          dynamics = user.dynamics.order(:created_at => :desc).reject { |dynamic| DateTime.parse(dynamic.created_at.to_s).strftime('%Y-%-m') != time}
          present paginate(Kaminari.paginate_array(dynamics)), with: ::Entities::Dynamic
        end
      end
