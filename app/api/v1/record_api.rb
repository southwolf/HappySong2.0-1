module V1
  class RecordApi < Grape::API
    ...
    resources :records do
      ...
      get '/my_records' do
        authenticate!
        records = current_user.records.reverse.group_by{ |record| DateTime.parse(record.created_at.to_s).strftime('%Y-%-m')}.to_a.reverse
      ...
      get '/other_records' do
        ...
        records = user.records.where(is_public: true).reverse.group_by{ |record| DateTime.parse(record.created_at.to_s).strftime('%Y-%-m')}.to_a.reverse
        present paginate(Kaminari.paginate_array(records)), with: ::Entities::HashRecord
      end
      desc "获取教师朗读总篇数"
      params do
